...
const ContactContent = () => (
  <div className="flex flex-col">
    <div className="grid grid-cols-12 border-b border-border w-full font-chicago-kare md:text-lg select-none">
      <div className="col-span-3 border-r border-border px-2 inset-shadow-bevel">System</div>

      <div className="col-span-9 px-2 inset-shadow-bevel">Username</div>
    </div>

    {CONTACT_OPTIONS.map(({ baseUrl, icon: Icon, label, slug, username }) => (
      <div className="grid-cols-12 grid bg-secondary/30 not-first:mb-0.5 items-center" key={slug}>
        ...
        <div className="col-span-9 flex items-center justify-between px-1 md:px-2 gap-1 md:gap-4">
          ...
          <ContactOpenButton baseUrl={baseUrl} label={label} username={username} />
        </div>
      </div>
    ))}
  </div>
